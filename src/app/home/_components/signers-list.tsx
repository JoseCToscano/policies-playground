...
export function SignersList({ walletId, onAttachPolicy }: {
    ...
    const handleDownloadKeys = (publicKey: string) => {
        try {
            ...
            const content = `Wallet Key: ${walletId}\n\nSecret Key: ${secretKey}`;
            const blob = new Blob([content], { type: 'text/plain' });
            const url = window.URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = url;
            link.download = `agent-keys.txt`;
                            ...
                            <div className="relative flex flex-col border border-gray-200 rounded-lg bg-white p-3 transition-shadow hover:shadow-sm">
                                ...
                                <div className="flex items-start justify-between gap-2">
                                    ...
                                    <div className="flex items-start gap-2.5 min-w-0 flex-1">
                                        <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gray-50 border border-gray-200">
                                        ...
                                        <div className="flex flex-col min-w-0">
                                            <div className="flex items-center gap-1.5 min-w-0">
                                                <h3 className="text-sm font-medium text-gray-900 truncate">{signer.name}</h3>
                                                <span className="text-[10px] text-gray-400 mt-px shrink-0">•</span>
                                                <div className="flex items-center gap-1 min-w-0">
                                                    <span className="font-mono text-[11px] text-gray-500 truncate">{shortAddress(signer.publicKey)}</span>
                                                    <button
                                            ...
                                            <p className="text-[11px] text-gray-500 truncate">{signer.purpose}</p>
                                        </div>
                                    </div>
                                    ...
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <button className="rounded-full p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition-colors shrink-0">
